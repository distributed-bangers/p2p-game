    ...
    for (const id of Object.keys(gameClient.state)) {
        ...
        for (const bullet of player.bullets) {
            ...
            for (const enemy of enemies) {
                if (enemy === gameClient.player) {
                    const collisionResults = raycaster.intersectObject(enemy)

                    if (collisionResults.length > 0 && collisionResults[0].distance < 5) {
                        const playerDirection = new THREE.Vector3()
                        gameClient.player.getWorldDirection(playerDirection)
                        const direction2 = new THREE.Vector3(direction.z, direction.y, direction.x)
                        const angle = direction.angleTo(playerDirection)
                        const angle2 = direction2.angleTo(playerDirection)
                        const lul = Math.PI / 2
                        if ((angle < lul && angle2 < lul) || (angle > lul && angle2 > lul)) {
                            gameClient.player.state.moveLeft = true
                        } else {
                            gameClient.player.state.moveRight = true
                        }
                    ...
                    if (collisionResults.length > 0 && collisionResults[0].distance < 0.55) {
